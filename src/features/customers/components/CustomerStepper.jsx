// tmsui/src/features/customers/components/CustomerStepper.jsx
...
/**
 * CustomerStepper Component
 * - Manages the multi-step process for creating or editing a customer.
 * - Handles the flow of steps, form submission, and API calls.
 * 
 * @param {Customer} initialCustomerData - Initial customer data for edit mode.
 * @param {boolean} isEditMode - Indicates whether the form is in edit or create mode.
 * @returns {JSX.Element} The rendered CustomerStepper component.
 */
const CustomerStepper = ({ initialCustomerData = new Customer(), isEditMode = false }) => {
    const [activeStep, setActiveStep] = useState(0); // Tracks the current step in the stepper
    const [customerData, setCustomerData] = useState(initialCustomerData); // Stores the customer data being created or edited
    ...
    const { createCustomer, updateCustomer } = useCustomerApi(); // Hooks for API calls
    ...
    const handleFormSubmit = async () => {
        try {
            let result;
            if (isEditMode) {
                result = await updateCustomer(customerData.id, customerData);
            } else {
                result = await createCustomer(customerData);
            }

            const savedCustomer = Customer.fromApiResponse(result);
            setCustomerData(savedCustomer);
            setApiSuccess(true);
            setResponseMessage(`Successfully ${isEditMode ? 'updated' : 'created'} Customer with ID: ${savedCustomer.id}`);
    ...
    }, [activeStep, customerData, apiSuccess, responseMessage, errorResponse, isEditMode]);
