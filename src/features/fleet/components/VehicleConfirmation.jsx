// tmsui/src/features/fleet/components/VehicleConfirmation.jsx
...
/**
 * VehicleConfirmation Component
 * - This component displays the confirmation step after vehicle creation or update.
 * - It shows either a success or error message and, in the case of success, displays the details of the vehicle.
 * 
 ...
 * @param {Object} vehicleData - The data of the created or updated vehicle.
 * @param {ApiErrorResponse} errorResponse - The error response object containing field-specific errors.
 * @returns {JSX.Element} A confirmation screen displaying the result of the vehicle creation or update process.
 */
const VehicleConfirmation = ({ message, apiSuccess, vehicleData, errorResponse }) => {
    ...
    return (
        <Box>
                        ...
                        {Object.entries(fieldErrors).map(([field, errors]) => (
                            <ListItem key={field} disablePadding>
                                <ListItemText
                                    ...
                                    secondary={Array.isArray(errors) ? errors.join(', ') : errors} // Handle multiple errors for a field
                                />
                            </ListItem>
                        ))}
                    </List>
            ...
            {/* Display vehicle details if the API call was successful */}
            {apiSuccess && (
                <Grid container spacing={2} sx={{ mb: 3 }}>
                    <VehicleDetails vehicle={vehicleData} mode="view" /> {/* Use the view mode for VehicleDetails */}
                </Grid>
            )}

            {/* Navigation button to return to the vehicle list */}
            <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }}>
                <AnimateButton>
                    <Button
                        onClick={() => navigate('/md/fleet/vehicles/list')} // Navigate back to the vehicle list
                        variant="contained"
                        color="primary"
                    >
                        OK
